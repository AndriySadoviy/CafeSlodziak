...
/**
 * Notify cafe about new order.
 * Order of attempts: Telegram → SMTP → FormSubmit
 */
export async function sendOrderEmail(order) {
  ...
  // 1) Telegram (najpewniejsze na Netlify)
  try {
  ...
  // 3) FormSubmit (wymaga jednorazowej aktywacji w mailu)
  try {
  ...
  if (channels.length > 0) {
    return { sent: true, to, via: channels.join("+") };
  }

  return {
    sent: false,
    reason: "all_failed",
    message: errors.join(" | ") || "Brak kanału powiadomień",
    to,
    hint:
      "Ustaw TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID w Netlify LUB SMTP_* LUB aktywuj FormSubmit w mailu.",
  };
}